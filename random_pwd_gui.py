...
class pwd_app(ctk.CTk):
	def __init__(self, *args, **kwargs):
		...
		self.pwd_len_label = ctk.CTkLabel(self, text="Password Length: "+str(self.pwd_len), text_color="white")
		...
		self.option_upc = ctk.CTkCheckBox(self, text="Uppercase", variable=self.add_uppercase, onvalue=True, offvalue=False)
		self.option_upc.pack(padx=10, pady=10)
		self.option_upc.select()

		# option: Lowercase
		self.add_lowercase = ctk.BooleanVar(value=True)
		self.option_loc = ctk.CTkCheckBox(self, text="Lowercase", variable=self.add_lowercase, onvalue=True, offvalue=False)
		self.option_loc.pack(padx=10, pady=10)
		self.option_loc.select()

		# option: Number
		self.add_number = ctk.BooleanVar(value=True)
		self.option_num = ctk.CTkCheckBox(self, text="Number", variable=self.add_number, onvalue=True, offvalue=False)
		self.option_num.pack(padx=10, pady=10)				
		self.option_num.select()

		# option: Symbol
		self.add_symbol = ctk.BooleanVar(value=True)
		self.option_sym = ctk.CTkCheckBox(self, text="Symbol", variable=self.add_symbol, onvalue=True, offvalue=False)
	...
	def slider_event(self, value):
		...
		self.pwd_len_label.configure(text = "Password Length: "+str(self.pwd_len), text_color="white")
		return
	...
	def pwd_generation(self):
		...
		if (len(selected_option)==0):
			# error message
			self.pwd_len_label.configure(text = "[ERROR] Please select at least one option!", text_color="red")
			return
